...
CubeL3DRenderer.prototype.render = function(cubeData) {
    ...
    if (this.cube.cells)
    {   // dealing with a traditional cube
        this._loadBufferFromCube();
    } else if (this.cube instanceof Array)
    {
        this._loadBufferFromArray();
    }

    // console.log('rbufft', new Date().getTime() - t0);

    this._np.animate(512, this._buffer, this._renderStep.bind(this));
    return this;
};

CubeL3DRenderer.prototype._loadBufferFromArray = function() {
    for (var i = 0, numBytes = this._buffer.length; i < numBytes; i++)
    {
        this._buffer[i] = this.cube[i];
    }
    return this;
};

CubeL3DRenderer.prototype._loadBufferFromCube = function() {
